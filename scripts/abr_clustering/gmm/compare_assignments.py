...
def compare_assignments():
    # Load both datasets
    original_space = pd.read_csv(
        "results/euclidean_assignment_original_space/gene_cluster_associations_original_space.csv"
    )
    euclidean = pd.read_csv(
        "results/euclidean_assignment/gene_cluster_associations_euclidean.csv"
    )

    # Merge on analysis_key to compare
    merged = original_space.merge(
        euclidean[["analysis_key", "dominant_cluster_euclidean"]],
        on="analysis_key",
        how="inner",
    )
    ...
    changed = merged[
        merged["dominant_cluster_original_space"]
        != merged["dominant_cluster_euclidean"]
    ].copy()

    print(
        f"Genes changed between euclidean individual vs original-space approaches: {len(changed)} / {len(merged)}"
    )
    print()
    ...
    for old_cluster in sorted(changed["dominant_cluster_euclidean"].unique()):
        ...
        for new_cluster in sorted(subset["dominant_cluster_original_space"].unique()):
            count = len(
                subset[subset["dominant_cluster_original_space"] == new_cluster]
            )
            genes = subset[subset["dominant_cluster_original_space"] == new_cluster][
                "gene_symbol"
            ].tolist()
            print(
                f'  → Cluster {new_cluster}: {count} genes ({", ".join(genes[:5])}{"..." if len(genes) > 5 else ""})'
            )
