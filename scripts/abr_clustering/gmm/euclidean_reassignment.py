...
def load_all_data():
    ...
    cluster_centers = np.load(
        BASE_DIR / "results" / "june_23_2025" / "gmm_k4_tied" / "cluster_centers.npy"
    )

    # Load original ABR data
    original_data = pd.read_csv(
        BASE_DIR / "results" / "june_23_2025" / "gmm_k4_tied" / "original_data.csv"
    )

    # Load original gene associations for comparison
    orig_associations = pd.read_csv(
        BASE_DIR
        / "results"
        / "gene_cluster_analysis_sex_specific"
        / "gene_cluster_associations.csv"
    )
            ...
            entry = {
                ...
                **cluster_dist,
                "cluster_distribution": str(
                    {i: cluster_counts.get(i, 0) for i in range(4)}
                ),
                **sex_specific_metrics,
...
def plot_gene_abr_profiles(
    ...
    for cluster_id in range(4):
        ...
        ax.plot(
            FREQ_LABELS,
            cluster_means_original[cluster_id],
...
def main(plots_only=False):
    ...
    # Load all data
    metadata, normalized_data, cluster_centers, original_data, orig_associations = (
        load_all_data()
    )

    if not plots_only:
        print("Performing Euclidean distance-based cluster assignment...")
        euclidean_labels, distances = assign_clusters_euclidean(
            normalized_data, cluster_centers
        )

        # Compare with original assignments
        orig_labels = np.load(
            BASE_DIR / "results" / "june_23_2025" / "gmm_k4_tied" / "cluster_labels.npy"
        )
        changes = euclidean_labels != orig_labels
        print(
            f"Cluster assignments changed for {changes.sum()} / {len(changes)} mice ({changes.sum()/len(changes)*100:.1f}%)"
        )

        print("Creating gene associations with Euclidean assignments...")
        euclidean_associations_df = create_gene_associations_euclidean(
            metadata, euclidean_labels, orig_associations
        )

        # Save associations CSV
        euclidean_associations_df.to_csv(
            OUTPUT_DIR / "gene_cluster_associations_euclidean.csv", index=False
        )
        print(
            f"Saved associations to {OUTPUT_DIR / 'gene_cluster_associations_euclidean.csv'}"
        )
    else:
        print("Loading existing Euclidean assignments...")
        euclidean_associations_df = pd.read_csv(
            OUTPUT_DIR / "gene_cluster_associations_euclidean.csv"
        )
        euclidean_labels, _ = assign_clusters_euclidean(
            normalized_data, cluster_centers
        )

    # Calculate cluster averages in original scale
    cluster_means_original = calculate_cluster_averages_original(
        original_data, euclidean_labels
    )

    # Create plots for each gene/allele combination
    print(
        f"Creating plots for {len(euclidean_associations_df)} gene/allele combinations..."
    )
    ...
    for idx, gene_data in euclidean_associations_df.iterrows():
        ...
        filename_base = (
            f"{gene_symbol}_{allele_short}_{zygosity}_{center}_{analysis_type}"
        )
        output_path_base = PLOT_DIR / filename_base

        try:
            plot_gene_abr_profiles(
                gene_data,
                metadata,
                original_data,
                euclidean_labels,
            ...
            if (idx + 1) % 10 == 0:
                print(
                    f"  Processed {idx + 1} / {len(euclidean_associations_df)} plots..."
                )
    ...
    if not plots_only:
        ...
        summary = {
            ...
            "assignments_changed": int(
                euclidean_associations_df["changed_assignment"].sum()
            ),
            "mean_consistency_euclidean": float(
                euclidean_associations_df["consistency_score_euclidean"].mean()
            ),
            "mean_consistency_original": float(
                euclidean_associations_df["consistency_score_original"].mean()
            ),
            "genes_with_improved_consistency": int(
                (
                    euclidean_associations_df["consistency_score_euclidean"]
                    > euclidean_associations_df["consistency_score_original"]
                ).sum()
            ),
        }
        ...
        print(
            f"  Mean consistency (Euclidean): {summary['mean_consistency_euclidean']:.3f}"
        )
        print(
            f"  Mean consistency (Original): {summary['mean_consistency_original']:.3f}"
        )
        print(
            f"  Genes with improved consistency: {summary['genes_with_improved_consistency']}"
        )
