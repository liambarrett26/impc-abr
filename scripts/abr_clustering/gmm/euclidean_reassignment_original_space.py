...
def create_gene_associations_original_space(
    ...
    for _, gene_row in orig_associations.iterrows():
        ...
        if len(matching_mice) > 0:
                        ...
                        sex_specific_metrics.update(
                            {
                                ...
                                f"{sex}_cluster_0": (
                                    len(sex_mice) if dominant_cluster == 0 else 0
                                ),
                                f"{sex}_cluster_1": (
                                    len(sex_mice) if dominant_cluster == 1 else 0
                                ),
                                f"{sex}_cluster_2": (
                                    len(sex_mice) if dominant_cluster == 2 else 0
                                ),
                                f"{sex}_cluster_3": (
                                    len(sex_mice) if dominant_cluster == 3 else 0
                                ),
                            }
                        )
            ...
            cluster_dist = {
                f"cluster_{i}": n_total if i == dominant_cluster else 0
                for i in range(4)
            }
            ...
            entry = {
                ...
                **cluster_dist,
                "cluster_distribution": str(
                    {i: n_total if i == dominant_cluster else 0 for i in range(4)}
                ),
                "distances_to_clusters_original_space": str(
                    {k: round(v, 2) for k, v in distances_to_centers.items()}
                ),
                ...
                **sex_specific_metrics,
                ...
                "changed_from_original": dominant_cluster
                != gene_row["dominant_cluster"],
            }
...
def plot_gene_abr_profiles_original_space(
...
def main():
    ...
    cluster_means_original = calculate_cluster_means_original_space(
    ...
    original_space_associations_df.to_csv(
        OUTPUT_DIR / "gene_cluster_associations_original_space.csv", index=False
    )
    print(
        f"Saved associations to {OUTPUT_DIR / 'gene_cluster_associations_original_space.csv'}"
    )

    # Create plots
    print(
        f"Creating plots for {len(original_space_associations_df)} gene/allele combinations..."
    )
    ...
    for idx, gene_data in original_space_associations_df.iterrows():
        ...
        try:
            plot_gene_abr_profiles_original_space(
                gene_data,
                metadata,
                original_data,
                cluster_means_original,
            ...
            if gene_symbol in problem_genes:
                ...
                print(
                    f"  {gene_symbol}: Cluster {gene_data['dominant_cluster_original_space']} "
            ...
            if (idx + 1) % 10 == 0:
                print(
                    f"  Processed {idx + 1} / {len(original_space_associations_df)} plots..."
                )
    ...
    summary = {
        ...
        "changed_from_original": int(
            original_space_associations_df["changed_from_original"].sum()
        ),
    ...
    for gene in problem_genes:
        gene_data = original_space_associations_df[
            original_space_associations_df["gene_symbol"] == gene
        ]
    ...
    for gene, info in summary["problem_genes_analysis"].items():
        print(
            f"  {gene}: {info['original_cluster']} → {info['original_space_cluster']} (distances: {info['distances']})"
        )
