...
class ParallelGMMPipeline:
    ...
    def load_preprocessed_data(self) -> tuple:
        ...
        if not normalized_data_path.exists():
            raise FileNotFoundError(
                f"Preprocessed data not found at {normalized_data_path}"
            )
    ...
    def train_model(
        ...
        """
        self.logger.info(
            f"Training GMM with k={self.n_components}, "
        ...
        try:
            ...
            self._save_model_results(
                gmm, cluster_labels, cluster_probabilities, metrics, training_time
            )
    ...
    def run_analysis(
        ...
        self.logger.info(
            f"Analysis complete, {len(visualization_files)} visualizations created"
        )
...
def main():
    ...
    # Required arguments
    parser.add_argument("n_components", type=int, help="Number of GMM components")
    ...
    parser.add_argument(
        "--output-dir", type=str, default="results", help="Base output directory"
    )
    ...
    if results.get("success", False):
        ...
        print(
            f"Model {pipeline.model_name} failed: {results.get('error', 'Unknown error')}"
        )
        sys.exit(1)
