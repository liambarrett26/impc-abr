...
def load_data(assignment_method="probabilistic"):
    ...
    if assignment_method == "euclidean":
        ...
        cluster_probs = np.load(
            "results/june_23_2025/gmm_k4_tied/cluster_probabilities.npy"
        )
        original_data = None
...
def filter_significant_genes(bayesian_df, bf_threshold=3.0, p_threshold=0.5):
    ...
    all_sig = (bayesian_df["all_bayes_factor"] >= bf_threshold) & (
    ...
    def get_primary_analysis(row):
        bf_values = {
            ...
            "male": (
                row["male_bayes_factor"] if pd.notna(row["male_bayes_factor"]) else 0
            ),
            "female": (
                row["female_bayes_factor"]
                if pd.notna(row["female_bayes_factor"])
                else 0
            ),
        }
        # Only consider significant effects
        if not (
            row["all_bayes_factor"] >= bf_threshold
            and row["all_p_hearing_loss"] >= p_threshold
        ):
            bf_values["all"] = 0
        if not (
            pd.notna(row["male_bayes_factor"])
            and row["male_bayes_factor"] >= bf_threshold
            and row["male_p_hearing_loss"] >= p_threshold
        ):
            bf_values["male"] = 0
        if not (
            pd.notna(row["female_bayes_factor"])
            and row["female_bayes_factor"] >= bf_threshold
            and row["female_p_hearing_loss"] >= p_threshold
        ):
            bf_values["female"] = 0

        return max(bf_values.items(), key=lambda x: x[1])[0]

    sig_genes_df["primary_analysis_type"] = sig_genes_df.apply(
        get_primary_analysis, axis=1
    )
    ...
    for analysis_type in ["all", "male", "female"]:
        type_df = sig_genes_df[
            sig_genes_df["primary_analysis_type"] == analysis_type
        ].copy()
        if len(type_df) > 0:
...
def create_gene_mouse_mapping(
    sig_genes_df,
    metadata_df,
    cluster_labels,
    analysis_type="all",
    assignment_method="probabilistic",
    original_data=None,
):
    ...
    if assignment_method == "euclidean":
        ...
        cluster_labels = compute_euclidean_clusters(
            original_data, cluster_means_original
        )
    ...
    for _, gene_row in sig_genes_df.iterrows():
        ...
        if len(matching_mice) > 0:
            ...
            mapping_entry = {
                ...
                "analysis_key": gene_row.get(
                    key_col,
                    f"{gene_symbol}_{allele_symbol}_{zygosity}_{center}_{analysis_type}",
                ),
...
def analyze_cluster_patterns(gene_cluster_df, analysis_results):
    ...
    # Map cluster IDs to their characteristics - keep internal mapping but not used in visualization
    cluster_patterns = {
        0: "moderate_HL",
        1: "control/normal",
    ...
    gene_cluster_df["cluster_pattern"] = gene_cluster_df["dominant_cluster"].map(
        cluster_patterns
    )
    ...
    summary_stats = {
        ...
        "genes_per_cluster": gene_cluster_df["dominant_cluster"]
        .value_counts()
        .to_dict(),
        "mean_consistency_score": gene_cluster_df["consistency_score"].mean(),
        "consistency_by_cluster": gene_cluster_df.groupby("dominant_cluster")[
            "consistency_score"
        ]
        .agg(["mean", "std"])
        .to_dict(),
        "high_consistency_genes": len(
            gene_cluster_df[gene_cluster_df["consistency_score"] >= 0.8]
        ),
        "mixed_phenotype_genes": len(
            gene_cluster_df[gene_cluster_df["consistency_score"] < 0.6]
        ),
    }
...
def create_visualizations(
    gene_cluster_df, output_dir, all_genes_df=None, show_best_fit=True
):
    """Create visualizations of gene-cluster associations.

    Args:
        gene_cluster_df: DataFrame with significant genes
        output_dir: Output directory for plots
        all_genes_df: Optional DataFrame with all tested genes (for additional plot)
        show_best_fit: Whether to show line of best fit on scatter plots
    """
    ...
    # Use cluster numbers starting at 1
    cluster_labels = [f"Cluster {i+1}" for i in range(4)]

    # Use same color palette as visualize_results.py
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "gene_cluster_distribution.png", dpi=1200)
    plt.savefig(output_dir / "gene_cluster_distribution.eps", format="eps")
    plt.close()
    ...
    for cluster in range(4):
        cluster_data = gene_cluster_df[gene_cluster_df["dominant_cluster"] == cluster][
            "consistency_score"
        ]
        data_for_plot.append(cluster_data)

    parts = ax.violinplot(
        data_for_plot, positions=range(4), showmeans=True, showmedians=True
    )
    ...
    ax.axhline(
        y=0.8, color="r", linestyle="--", alpha=0.5, label="High consistency threshold"
    )
    ax.legend()

    plt.tight_layout()
    plt.savefig(output_dir / "gene_consistency_by_cluster.png", dpi=1200)
    plt.savefig(output_dir / "gene_consistency_by_cluster.eps", format="eps")
    plt.close()
    ...
    heatmap_data = gene_cluster_df[
        ["gene_symbol", "cluster_0", "cluster_1", "cluster_2", "cluster_3"]
    ].copy()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "gene_cluster_heatmap.png", dpi=1200)
    plt.savefig(output_dir / "gene_cluster_heatmap.eps", format="eps")
    plt.close()

    # 4. Scatter plot: Bayes Factor vs Consistency Score (Significant genes)
    fig, ax = plt.subplots(1, 1, figsize=(10, 8))

    # Use same color palette as visualize_results.py
    colors = sns.color_palette("husl", 4)

    # Plot all data points with same marker (circle)
    for cluster in range(4):
        cluster_data = gene_cluster_df[gene_cluster_df["dominant_cluster"] == cluster]
        if len(cluster_data) > 0:
            ax.scatter(
                cluster_data["bayes_factor"],
    ...
    if show_best_fit and len(gene_cluster_df) > 1:
        from scipy import stats

        # Log transform BF for linear regression
        log_bf = np.log10(gene_cluster_df["bayes_factor"].values)
        consistency = gene_cluster_df["consistency_score"].values

        # Remove any inf values
        mask = np.isfinite(log_bf)
        log_bf = log_bf[mask]
        ...
        if len(log_bf) > 1:
            ...
            x_fit = np.linspace(log_bf.min(), log_bf.max(), 100)
            y_fit = slope * x_fit + intercept
            # Transform x back to original scale
            x_fit_original = 10**x_fit
            ax.plot(
                x_fit_original,
                y_fit,
                "k--",
    ...
    ax.set_ylabel("Consistency Score")
    ax.set_xscale("log")
    ax.set_title("Gene Effect Size vs Phenotypic Consistency (Significant Genes)")
    ax.legend(loc="best")
    ax.grid(True, alpha=0.3)

    plt.tight_layout()
    plt.savefig(output_dir / "bayes_factor_vs_consistency_significant.png", dpi=1200)
    plt.savefig(
        output_dir / "bayes_factor_vs_consistency_significant.eps", format="eps"
    )
    plt.close()

    # 4b. Create separate plot for all tested genes if provided
    if all_genes_df is not None and len(all_genes_df) > 0:
        fig, ax = plt.subplots(1, 1, figsize=(10, 8))

        for cluster in range(4):
            cluster_data = all_genes_df[all_genes_df["dominant_cluster"] == cluster]
            if len(cluster_data) > 0:
                ax.scatter(
                    cluster_data["bayes_factor"],
                    cluster_data["consistency_score"],
        ...
        # Add line of best fit if requested
        if show_best_fit and len(all_genes_df) > 1:
            # Log transform BF for linear regression
            log_bf = np.log10(all_genes_df["bayes_factor"].values)
            consistency = all_genes_df["consistency_score"].values
        ...
        ax.set_ylabel("Consistency Score")
        ax.set_xscale("log")
        ax.set_title("Gene Effect Size vs Phenotypic Consistency (All Tested Genes)")
        ax.legend(loc="best")
        ax.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig(output_dir / "bayes_factor_vs_consistency_all.png", dpi=1200)
        plt.savefig(output_dir / "bayes_factor_vs_consistency_all.eps", format="eps")
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(output_dir / "cluster_distribution_by_sex.png", dpi=1200)
    plt.savefig(output_dir / "cluster_distribution_by_sex.eps", format="eps")
    plt.close()
...
def generate_report(gene_cluster_df, summary_stats, output_dir):
    ...
    report.append("-" * 40)
    report.append(
        f"Total significant genes analyzed: {summary_stats['total_significant_genes']}"
    )
    report.append(
        f"Mean consistency score: {summary_stats['mean_consistency_score']:.3f}"
    )
    report.append(
        f"Genes with high consistency (≥0.8): {summary_stats['high_consistency_genes']}"
    )
    report.append(
        f"Genes with mixed phenotypes (<0.6): {summary_stats['mixed_phenotype_genes']}"
    )
    report.append("")
    ...
    for cluster, count in sorted(summary_stats["genes_per_cluster"].items()):
        ...
        report.append(f"Cluster {cluster + 1}: {count} genes ({pct:.1f}%)")
    report.append("")
    ...
    for cluster in range(4):
        if cluster in summary_stats["consistency_by_cluster"]:
            ...
            report.append(f"Cluster {cluster + 1}:")
            report.append(
                f"  Mean consistency: {stats['mean']:.3f} ± {stats['std']:.3f}"
            )
    report.append("")
    ...
    for cluster in range(4):
        ...
        ].nlargest(5, "bayes_factor")
        report.append(f"\nCluster {cluster + 1}:")
        for _, gene in cluster_genes.iterrows():
            report.append(
                f"  {gene['gene_symbol']} - BF: {gene['bayes_factor']:.1f}, "
    ...
    report.append("-" * 40)
    mixed_genes = gene_cluster_df[gene_cluster_df["consistency_score"] < 0.6].nlargest(
        10, "bayes_factor"
    )
    for _, gene in mixed_genes.iterrows():
        ...
        report.append(
            f"{gene['gene_symbol']} - Consistency: {gene['consistency_score']:.2f}, "
    ...
    if len(all_genes) > 0 and "male_consistency_score" in all_genes.columns:
        ...
        sex_diff_genes = all_genes[
            ...
            & (
                all_genes["male_dominant_cluster"]
                != all_genes["female_dominant_cluster"]
            )
        ]
        ...
        if len(sex_diff_genes) > 0:
            ...
            for _, gene in sex_diff_genes.nlargest(10, "bayes_factor").iterrows():
                male_cluster = (
                    int(gene["male_dominant_cluster"])
                    if pd.notna(gene["male_dominant_cluster"])
                    else "N/A"
                )
                female_cluster = (
                    int(gene["female_dominant_cluster"])
                    if pd.notna(gene["female_dominant_cluster"])
                    else "N/A"
                )
                report.append(
                    f"  {gene['gene_symbol']} - Male: Cluster {male_cluster}, Female: Cluster {female_cluster}"
                )
...
def show_sex_specific_genes(output_dir="results/gene_cluster_analysis_sex_specific"):
    ...
    sex_specific = df[df["analysis_type"] != "all"].sort_values(
        ["analysis_type", "bayes_factor"], ascending=[True, False]
    )
    ...
    for analysis_type in ["male", "female"]:
        ...
        if len(type_df) > 0:
            ...
            for _, gene in type_df.iterrows():
                ...
                print(f"    - Dominant cluster: Cluster {gene['dominant_cluster'] + 1}")
                print(f"    - Consistency: {gene['consistency_score']:.2f}")
                print(f"    - Bayes Factor: {gene['bayes_factor']:.1f}")
                print(f"    - N mice: {gene['n_total_mice']}")
                print(
                    f"    - Distribution: C1:{gene['cluster_0']}, C2:{gene['cluster_1']}, C3:{gene['cluster_2']}, C4:{gene['cluster_3']}"
                )
            print()
...
def show_sex_differences_only(output_dir="results/gene_cluster_analysis_sex_specific"):
    ...
    sex_specific_only = df[
        (df["analysis_type"].isin(["male", "female"])) & (~df["sig_in_all"])
    ]
    ...
    if len(sex_specific_only) > 0:
        ...
        for _, gene in sex_specific_only.iterrows():
            print(
                f"{gene['gene_symbol']} ({gene['center']}) - {gene['analysis_type'].upper()}-specific"
            )
            print(f"  - Dominant cluster: Cluster {gene['dominant_cluster'] + 1}")
            print(
                f"  - {gene['analysis_type'].capitalize()} BF: {gene['bayes_factor']:.1f}"
            )
    ...
    sig_patterns = (
        df.groupby(["sig_in_all", "sig_in_male", "sig_in_female"])
        .size()
    ...
    multi_sig = df[
        (
            df["sig_in_all"].astype(int)
            + df["sig_in_male"].astype(int)
            + df["sig_in_female"].astype(int)
        )
        > 1
    ]
...
def demo_csv_structure(output_dir="results/gene_cluster_analysis_sex_specific"):
    ...
    columns_to_show = [
        ...
        "consistency_score",
        "male_n_mice",
        "male_dominant_cluster",
        "male_consistency_score",
        "female_n_mice",
        "female_dominant_cluster",
        "female_consistency_score",
    ]
...
def main(mode="full", assignment_method="probabilistic", output_dir=None):
    ...
    if output_dir is None:
        if assignment_method == "euclidean":
            output_dir = Path(
                "results/euclidean_assignment_original_space/analysis_plots"
            )
        else:
    ...
    for sig_genes_df, analysis_type in significant_groups:
        ...
        print(
            f"Found {len(sig_genes_df)} significant gene combinations for {analysis_type}"
        )
        ...
        print(
            f"Successfully mapped {len(mapping_df)} gene groups to clusters for {analysis_type}"
        )

        all_mappings.append(mapping_df)

    # Also create mapping for all tested genes (not just significant)
    print("\nProcessing all tested genes...")
    all_tested_mappings = []
    for analysis_type in ["all", "male", "female"]:
        type_df = bayesian_df.copy()
        type_df["analysis_type"] = analysis_type
    ...
    # Combine mappings
    gene_cluster_df = (
        pd.concat(all_mappings, ignore_index=True) if all_mappings else pd.DataFrame()
    )
    all_genes_cluster_df = (
        pd.concat(all_tested_mappings, ignore_index=True)
        if all_tested_mappings
        else pd.DataFrame()
    )
    ...
    gene_cluster_df, summary_stats = analyze_cluster_patterns(
        gene_cluster_df, analysis_results
    )

    # Add sex-specific summary statistics
    summary_stats["genes_by_analysis_type"] = (
        gene_cluster_df["analysis_type"].value_counts().to_dict()
    )
    summary_stats["consistency_by_analysis_type"] = (
        gene_cluster_df.groupby("analysis_type")["consistency_score"]
        .agg(["mean", "std"])
        .to_dict()
    )
    ...
    create_visualizations(
        gene_cluster_df, output_dir, all_genes_cluster_df, show_best_fit=True
    )
