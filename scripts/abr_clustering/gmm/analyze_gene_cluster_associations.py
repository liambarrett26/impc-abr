...
def create_visualizations(gene_cluster_df, output_dir, all_genes_df=None, show_best_fit=True):
    ...
    bars = ax.bar(cluster_counts.index, cluster_counts.values, color=[colors[i] for i in cluster_counts.index])
    ...
    if show_best_fit and len(gene_cluster_df) > 1:
        ...
        if len(log_bf) > 1:
            slope, intercept, r_value, p_value, std_err = stats.linregress(log_bf, consistency)
            x_fit = np.linspace(log_bf.min(), log_bf.max(), 100)
            y_fit = slope * x_fit + intercept
            # Transform x back to original scale
            x_fit_original = 10 ** x_fit
            ax.plot(x_fit_original, y_fit, 'k--', alpha=0.5, linewidth=2,
    ...
    plt.savefig(output_dir / 'bayes_factor_vs_consistency_significant.eps', format='eps')
    plt.close()
    ...
    for analysis_type in ['male', 'female']:
        ...
        if len(type_df) > 0:
            ...
            for _, gene in type_df.iterrows():
                ...
                print(f"    - Distribution: C1:{gene['cluster_0']}, C2:{gene['cluster_1']}, C3:{gene['cluster_2']}, C4:{gene['cluster_3']}")
            print()
...
def main(mode='full'):
    ...
    all_genes_cluster_df = pd.concat(all_tested_mappings, ignore_index=True) if all_tested_mappings else pd.DataFrame()
