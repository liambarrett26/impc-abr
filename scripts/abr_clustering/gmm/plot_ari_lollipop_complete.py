...
def plot_lollipop_horizontal(df, output_dir):
    ...
    ax.scatter(k_values, ari_scores, s=250, color=BLUE_COLOR,
               edgecolor='black', linewidth=2, zorder=3, alpha=0.9)

    # Add value labels above markers
    for k, score in zip(k_values, ari_scores):
        ax.text(k, score + 0.04, f'{score:.3f}',
    ...
    ax.set_title('Cluster Stability: Tied Covariance Models',
...
def main():
    # Configuration
    results_dir = Path('/home/liamb/impc-abr/scripts/abr_clustering/gmm/results/june_23_2025')
    ...
    print("-" * 40)
    print(f"  Best: k={df.loc[df['stability_score'].idxmax(), 'n_components']:.0f} (ARI = {df['stability_score'].max():.4f})")
    print(f"  Worst: k={df.loc[df['stability_score'].idxmin(), 'n_components']:.0f} (ARI = {df['stability_score'].min():.4f})")
    ...
    df[['n_components', 'stability_score', 'bic', 'silhouette']].to_csv(summary_path, index=False)
