...
def preprocess_and_save(
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(df)} samples from {len(experimental_groups) if experimental_groups else 0} groups"
        )
        ...
        logger.info(
            f"Clean dataset: {len(df_clean)} samples ({len(df_clean)/initial_count*100:.1f}% retained)"
        )
        ...
        available_cols = [
            col for col in metadata_cols + abr_cols if col in df_clean.columns
        ]
        ...
        preproc_info = {
            ...
            "n_experimental_groups": (
                len(experimental_groups) if experimental_groups else 0
            ),
        ...
        data_stats = {
            ...
            "metadata": {
                "n_unique_genes": (
                    int(metadata["gene_symbol"].nunique())
                    if "gene_symbol" in metadata
                    else 0
                ),
                "n_centers": (
                    int(metadata["phenotyping_center"].nunique())
                    if "phenotyping_center" in metadata
                    else 0
                ),
                "sex_distribution": (
                    metadata["sex"].value_counts().to_dict()
                    if "sex" in metadata
                    else {}
                ),
                "sample_group_distribution": (
                    metadata["biological_sample_group"].value_counts().to_dict()
                    if "biological_sample_group" in metadata
                    else {}
                ),
            },
        }
        ...
        logger.info(
            f"Preprocessing completed in {preproc_info['processing_time']:.1f} seconds"
        )
