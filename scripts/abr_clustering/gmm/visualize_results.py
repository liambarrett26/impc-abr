...
class ResultsVisualizer:
    ...
    def create_all_visualizations(self, output_dir: Optional[Path] = None, dpi: int = 1200):
        ...
        plot_files['audiogram_profiles'] = str(output_dir / "cluster_audiogram_profiles.png")
        ...
        plot_files['cluster_distributions'] = str(output_dir / "cluster_distributions.png")
        
        # 5. Assignment uncertainty
        fig = self.plot_uncertainty_heatmap()
        self._save_figure(fig, output_dir / "assignment_uncertainty", dpi)
        plot_files['uncertainty_heatmap'] = str(output_dir / "assignment_uncertainty.png")
        
        # 6. Gene associations (if available)
        if 'gene_associations' in self.analysis_results and self.analysis_results['gene_associations']:
            fig = self.plot_gene_associations()
            self._save_figure(fig, output_dir / "gene_cluster_associations", dpi)
            plot_files['gene_associations'] = str(output_dir / "gene_cluster_associations.png")
    ...
    def plot_cluster_profiles(self, use_original_scale: bool = True, use_confidence_intervals: bool = True) -> plt.Figure:
        """Plot mean audiogram profiles for each cluster."""
        data = self.original_data if (use_original_scale and self.original_data is not None) else self.normalized_data
        ...
        for cluster_id in range(self.n_clusters):
            ...
            pattern = self.analysis_results['cluster_characteristics'][str(cluster_id)]['pattern_type']
    ...
    def plot_cluster_distributions(self) -> plt.Figure:
        ...
        patterns = [self.analysis_results['cluster_characteristics'][str(i)]['pattern_type'] 
        ...
        confidence_by_cluster = [max_probs[self.cluster_labels == i] for i in range(self.n_clusters)]
    ...
    def plot_gene_associations(self, top_n: int = 20) -> plt.Figure:
        ...
        if not gene_data:
            ...
            ax.text(0.5, 0.5, 'No gene association data available', 
        ...
        axes[0].set_yticklabels([f"{g} (n={n})" for g, n in zip(gene_df['gene'], gene_df['n'])])
        axes[0].set_xlabel('Proportion in Dominant Cluster')
        axes[0].set_title(f'Top {len(gene_df)} Gene-Cluster Associations')
        axes[0].set_xlim(0, 1)
        
        # Add cluster labels
        for i, (cluster, prop) in enumerate(zip(gene_df['dominant_cluster'], gene_df['proportion'])):
        ...
        if gene_distributions:
            ...
            im = axes[1].imshow(gene_distributions, aspect='auto', cmap='Blues', vmin=0, vmax=1)
    ...
    def create_custom_figure(self, figure_type: str, **kwargs) -> plt.Figure:
        ...
        if figure_type not in figure_map:
            raise ValueError(f"Unknown figure type: {figure_type}. Choose from {list(figure_map.keys())}")
...
def main():
    ...
    parser.add_argument(
        '--figure-type',
        type=str,
        choices=['all', 'profiles', 'pca_clusters', 'pca_confidence', 'distributions', 'uncertainty', 'genes'],
        default='all',
    ...
    if args.figure_type == 'all':
        ...
        print(f"Created {args.figure_type} visualization: {base_path}.png and {base_path}.eps")
