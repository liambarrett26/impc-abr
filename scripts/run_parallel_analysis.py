...
from abr_analysis.analysis.parallel_executor import (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run parallel Bayesian analysis of ABR data"
    )
    parser.add_argument("--data", required=True, help="Path to the ABR data file")
    parser.add_argument(
        "--output", default="results/parallel_bayes", help="Output directory"
    )
    parser.add_argument(
        "--processes",
        type=int,
    ...
    print(
        f"Starting parallel Bayesian analysis with {args.processes if args.processes else 'auto'} processes"
    )
    ...
    if results_df is not None:
        create_visualizations(
            results_df,
            output_dir,
            "./data/multivariate_confirmed_deafness_genes.txt",
            "./data/multivariate_candidate_deafness_genes.txt",
            args.min_bf,
        )
