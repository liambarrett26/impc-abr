...
def generate_report(
    ...
    for analysis_type in ["combined", "all", "male", "female"]:
        ...
        significant_entries = get_significant_entries_detailed(
            df, analysis_type, threshold
        )
        overlap_analysis = analyze_gene_overlap(
            significant_genes, confirmed_genes, candidate_genes
        )
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for analysis_type in ["combined", "all", "male", "female"]:
            ...
            f.write(
                f"Total significant gene/allele combinations: {total_sig_entries}\n"
            )
            f.write(
                f"Found in confirmed deafness genes: {len(overlap['found_in_confirmed'])}\n"
            )
            f.write(
                f"Found in candidate deafness genes: {len(overlap['found_in_candidate'])}\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate ABR analysis report from detailed results"
    )
    parser.add_argument(
        "--results",
        "-r",
    ...
    parser.add_argument(
        "--output", "-o", help="Output file path (default: timestamped filename)"
    )
    parser.add_argument(
        "--threshold",
        "-t",
    ...
    try:
        output_path = generate_report(
            args.results, args.confirmed, args.candidate, args.output, args.threshold
        )
