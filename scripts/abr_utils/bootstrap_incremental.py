...
# -*- coding: utf-8 -*-
"""
Incremental Bootstrap Script for Missing Model Configurations

Saves models for individual analysis types (all/male/female) as soon as they match,
rather than waiting for all analyses to match in a single iteration.
...
class IncrementalBootstrapper:
    """Manages incremental saving of matched models."""
    ...
    def load_existing_progress(self):
        ...
        if progress_file.exists():
            ...
            print(
                f"  Loaded existing progress: {sum(len(allele.values()) for allele in self.saved_models.values())} models saved"
            )
    ...
    def copy_model_files(self, temp_dir, allele_key, analysis_type):
        ...
        # Find the model directory in temp_dir
        ...
        # Look for the specific analysis directory
        analysis_suffix_map = {"all": "_all", "male": "_male", "female": "_female"}
        suffix = analysis_suffix_map[analysis_type]

        # Find directory that ends with the right suffix
        source_dir = None
        ...
        if not source_dir:
            print(
                f"    Warning: Could not find model directory for {allele_key}_{analysis_type}"
            )
            return False
...
def extract_allele_stats(results):
    """Extract statistics organized by allele."""
    allele_stats = {}
...
def bootstrap_gene_incremental(gene, data_path, max_iterations):
    """Bootstrap with incremental saving."""
    ...
    # Create bootstrapper
    output_dir = f"results/bootstrap_incremental_{gene}"
    bootstrapper = IncrementalBootstrapper(gene, output_dir)
    ...
    for iteration in range(max_iterations):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                ...
                # Extract statistics by allele
                allele_results = extract_allele_stats(results)
                ...
                for target_allele_key, target_allele_stats in target_alleles.items():
                    if target_allele_key in allele_results:
                        ...
                        for (
                            analysis_type,
                            target_analysis_stats,
                        ) in target_allele_stats.items():
                            # Skip if already saved
                            if bootstrapper.is_already_saved(
                                target_allele_key, analysis_type
                            ):
                                continue

                            if analysis_type in result_allele_stats:
                                result_analysis_stats = result_allele_stats[
                                    analysis_type
                                ]
                                if check_stats_match(
                                    result_analysis_stats, target_analysis_stats
                                ):
                                    # Found a match! Save it immediately
                                    if bootstrapper.copy_model_files(
                                        temp_dir, target_allele_key, analysis_type
                                    ):
                                        bootstrapper.mark_as_saved(
                                            target_allele_key, analysis_type
                                        )
                                        new_saves += 1
                                        print(
                                            f"    🎯 MATCH: {target_allele_key[:25]}...{analysis_type} - p_hl={result_analysis_stats['p_hearing_loss']:.3f}"
                                        )
                                else:
                                    print(
                                        f"    ❌ {target_allele_key[:25]}...{analysis_type}: p_hl={result_analysis_stats['p_hearing_loss']:.3f} (target: {target_analysis_stats['p_hearing_loss']:.3f})"
                                    )
                            else:
                                print(
                                    f"    ❓ {target_allele_key[:25]}...{analysis_type}: No results"
                                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Incremental bootstrap for missing model configurations"
    )
    ...
    parser.add_argument(
        "--max-iterations", type=int, default=100, help="Maximum iterations per gene"
    )
    parser.add_argument(
    ...
    for gene in genes_to_process:
        success = bootstrap_gene_incremental(gene, str(data_path), args.max_iterations)
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("INCREMENTAL BOOTSTRAP SUMMARY")
    print(f"{'='*60}")
