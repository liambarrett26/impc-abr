...
class IncrementalBootstrapper:
    ...
    def copy_model_files(self, temp_dir, allele_key, analysis_type):
        ...
        if not source_dir:
            print(f"    Warning: Could not find model directory for {allele_key}_{analysis_type}")
            return False
                            ...
                            if analysis_type in result_allele_stats:
                                ...
                                if check_stats_match(result_analysis_stats, target_analysis_stats):
                                    ...
                                    if bootstrapper.copy_model_files(temp_dir, target_allele_key, analysis_type):
                                        ...
                                        new_saves += 1
                                        print(f"    🎯 MATCH: {target_allele_key[:25]}...{analysis_type} - p_hl={result_analysis_stats['p_hearing_loss']:.3f}")
                                else:
                                    print(f"    ❌ {target_allele_key[:25]}...{analysis_type}: p_hl={result_analysis_stats['p_hearing_loss']:.3f} (target: {target_analysis_stats['p_hearing_loss']:.3f})")
                            else:
                                print(f"    ❓ {target_allele_key[:25]}...{analysis_type}: No results")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Incremental bootstrap for missing model configurations")
