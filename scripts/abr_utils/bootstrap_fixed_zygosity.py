...
class ZygosityAwareBootstrapper:
    ...
    def find_specific_model_dir(self, temp_dir, target_allele_key, analysis_type):
        ...
        print(
            f"    Warning: Could not find model directory for {target_allele_key}_{analysis_type}"
        )
                            ...
                            if analysis_type in result_allele_stats:
                                ...
                                if check_stats_match(
                                    ...
                                    if bootstrapper.copy_model_files(
                                        ...
                                        new_saves += 1
                                        print(
                                            f"    🎯 MATCH: {target_allele_key[:30]}...{analysis_type}"
                                        )
                                        print(
                                            f"        p_hl={result_analysis_stats['p_hearing_loss']:.3f} (zygosity: {result_analysis_stats['zygosity']})"
                                        )
                                else:
                                    print(
                                        f"    ❌ {target_allele_key[:30]}...{analysis_type}: p_hl={result_analysis_stats['p_hearing_loss']:.3f} (target: {target_analysis_stats['p_hearing_loss']:.3f})"
                                    )
                            else:
                                print(
                                    f"    ❓ {target_allele_key[:30]}...{analysis_type}: No results"
                                )
                    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Zygosity-aware bootstrap for missing model configurations"
    )
    ...
    for gene in genes_to_process:
        success = bootstrap_gene_zygosity_aware(
            gene, str(data_path), args.max_iterations
        )
