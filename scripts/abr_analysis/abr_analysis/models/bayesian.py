...
class BayesianABRAnalysis:
    ...
    def fit(self, control_profiles, mutant_profiles, freq_cols):
        """Fit the Bayesian mixture model to control and mutant data.

        Builds the PyMC model and draws posterior samples (stored on
        ``self.trace``).

        Args:
            control_profiles (numpy.ndarray): Array of shape
                (n_controls, n_frequencies) of wild-type ABR thresholds.
            mutant_profiles (numpy.ndarray): Array of shape
                (n_mutants, n_frequencies) of knockout ABR thresholds.
            freq_cols (list[str]): Frequency column names, in ascending order.
        """
        ...
        with pm.Model() as self.model:
            ...
            control_sd = pm.HalfNormal("control_sd", sigma=10, shape=n_dims)
            ...
            hl_mean = pm.Deterministic("hl_mean", control_mean + hl_shift)

            # Binary indicator for each mutant
            z = pm.Bernoulli("z", p=p_hearing_loss, shape=n_mutants)

            # Model for mutants using independent normal distributions
            for i in range(n_dims):
                means_i = pm.math.switch(z, hl_mean[i], control_mean[i])
            ...
            self.trace = pm.sample(
                2000, tune=1000, target_accept=0.9, return_inferencedata=True
            )

    def calculate_bayes_factor(self):
        """Calculate the Bayes factor for hearing loss vs normal hearing.

        Returns:
            float: The posterior-to-prior odds ratio of ``p_hearing_loss > 0.5``,
            quantifying the evidence for hearing loss.
        """
    ...
    def get_summary_statistics(self):
        """Summarise the posterior for the key model parameters.

        Returns:
            pandas.DataFrame: ArviZ summary (mean, HDI, diagnostics) for
            ``p_hearing_loss`` and the per-frequency ``hl_shift`` terms.
        """
        summary = az.summary(self.trace, var_names=["p_hearing_loss", "hl_shift"])
        return summary

    def plot_results(self, control_profiles, mutant_profiles, gene_name=None):
        """Create a comprehensive four-panel visualisation of the results.

        Args:
            control_profiles (numpy.ndarray): Control ABR thresholds.
            mutant_profiles (numpy.ndarray): Mutant ABR thresholds.
            gene_name (str, optional): Gene label for the figure title.
    ...
    def _plot_posterior_distributions(self, ax):
        ...
        az.plot_posterior(self.trace, var_names=["p_hearing_loss"], ax=ax)
    ...
    def _plot_effect_size(self, ax):
        ...
        az.plot_forest(self.trace, var_names=["hl_shift"], ax=ax, combined=True)
    ...
    def save_model(self, directory):
        ...
        try:
            ...
            with open(readme_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"## Bayes Factor: {spec['parameters']['bayes_factor']:.2f}\n\n"
                )
                ...
                for freq, values in spec["hearing_loss_shifts"].items():
                    ...
                    f.write(
                        f"- 95% HDI: [{values['hdi_3%']:.2f}, {values['hdi_97%']:.2f}] dB\n\n"
                    )
