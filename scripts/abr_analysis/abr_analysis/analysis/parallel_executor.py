...
def process_experimental_group(group_info, data_path, save_dir=None):
    ...
    os.environ["PYTENSOR_FLAGS"] = (
        f"base_compiledir={temp_cache_dir},device=cpu,optimizer=fast_compile"
    )
        ...
        for analysis_type, analysis in analyses.items():
            if analysis:
                ...
                if save_dir:
                    ...
                    safe_allele = (
                        str(allele).replace("<", "").replace(">", "").replace("/", "_")
                    )
                    model_save_dir = (
                        model_dir / f"{safe_allele}_{zygosity}_{center}_{analysis_type}"
                    )

                    if analysis_key in analyzer.bayesian_models:
                        # DIRECT MODEL SAVING - Save immediately after creation
                        print(
                            f"Saving model for {group_desc} ({analysis_type}) to {model_save_dir}"
                        )
                        try:
                            ...
                            if save_success:
                                print(
                                    f"Successfully saved model for {group_desc} ({analysis_type})"
                                )

                                # Also create visualization if significant (BF > 3)
                                if (
                                    analysis["bayes_factor"] > 3
                                    and analysis_type == "all"
                                ):
                                    try:
                                        # Get control and mutant data
                                        control_data = analyzer.control_profiles[
                                            analysis_key
                                        ]
                                        mutant_data = analyzer.mutant_profiles[
                                            analysis_key
                                        ]

                                        # Extract profiles
                                        control_profiles = (
                                            analyzer.matcher.get_control_profiles(
                                        ...
                                        fig.suptitle(
                                            f"{gene} - {allele} ({zygosity}) - {center} ({analysis_type})",
                                            fontsize=14,
                                        )
                                        fig.tight_layout(
                                            rect=[0, 0, 1, 0.95]
                                        ...
                                        plt.close(fig)
                                        print(
                                            f"Successfully saved visualization for {group_desc} ({analysis_type})"
                                        )
                                    except Exception as viz_err:
                                        print(
                                            f"Error creating visualization for {group_desc} ({analysis_type}): {str(viz_err)}"
                                        )
                            else:
                                print(
                                    f"Failed to save model for {group_desc} ({analysis_type})"
                                )
                        except Exception as model_err:
                            print(
                                f"Error saving model for {group_desc} ({analysis_type}): {str(model_err)}"
                            )
                            traceback.print_exc()
                    else:
                        print(
                            f"Warning: Model for analysis key '{analysis_key}' not found in cache"
                        )
            else:
...
def run_parallel_analysis(
    ...
    mutants = temp_analyzer.data[
        temp_analyzer.data["biological_sample_group"] == "experimental"
    ]
    ...
    for batch_index, gene_batch in enumerate(gene_batches):
        ...
        with ProcessPoolExecutor(max_workers=n_processes) as executor:
            ...
            futures = {
                executor.submit(process_gene_groups, gene, data_path, output_dir): gene
                for gene in gene_batch
            }

            # Process results as they complete
            for future in tqdm(
                futures, total=len(gene_batch), desc=f"Batch {batch_index+1}"
            ):
                gene = futures[future]
                try:
        ...
        if batch_results:
            ...
            interim_df.to_csv(
                output_dir / f"interim_results_batch_{batch_index+1}.csv", index=False
            )
            print(
                f"Saved interim results ({len(interim_df)} experimental groups processed so far)"
            )
    ...
    if confirmed_genes_path.exists() and candidate_genes_path.exists():
        comparisons = compare_with_known_genes(
            gene_summary, confirmed_genes_path, candidate_genes_path, min_bf
        )
    ...
    create_visualizations(
        gene_summary, output_dir, confirmed_genes_path, candidate_genes_path, min_bf
    )
...
def compare_with_known_genes(
    results_df, confirmed_genes_path, candidate_genes_path, min_bf=3.0
):
    """
...
def write_summary_report(
    ...
    n_processes,
    min_bf,
    confirmed_genes_path=None,
    candidate_genes_path=None,
):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        for analysis_type in ["all", "male", "female"]:
            ...
            if bf_col in gene_summary.columns:
                ...
                if comparisons:
                    ...
                    f.write("NOVEL GENES:\n")
                    f.write(
                        f"New potential hearing loss genes: {len(comp['novel'])}\n\n"
                    )
        ...
        if "all_evidence" in gene_summary.columns:
            ...
            for evidence, count in evidence_counts.items():
                f.write(
                    f"{evidence}: {count} genes ({count/len(gene_summary)*100:.1f}%)\n"
                )
        ...
        for analysis_type in ["all", "male", "female"]:
            ...
            if (
                mutant_col in gene_summary.columns
                and control_col in gene_summary.columns
            ):
                f.write(f"\n{analysis_type.upper()}:\n")
                f.write(
                    f"Mean mutants per gene: {gene_summary[mutant_col].mean():.1f}\n"
                )
                f.write(
                    f"Mean controls per gene: {gene_summary[control_col].mean():.1f}\n"
                )
...
def create_visualizations(
    ...
    has_male_data = (
        "male_bayes_factor" in gene_summary.columns
        and not gene_summary["male_bayes_factor"].isna().all()
    )
    has_female_data = (
        "female_bayes_factor" in gene_summary.columns
        and not gene_summary["female_bayes_factor"].isna().all()
    )
    ...
    if "all_center" in gene_summary.columns:
        sig_with_center = gene_summary[
            (gene_summary["all_bayes_factor"] > min_bf)
    ...
    if (
        confirmed_genes_path
        and candidate_genes_path
        and Path(confirmed_genes_path).exists()
        and Path(candidate_genes_path).exists()
    ):
        # Load gene lists
        with open(confirmed_genes_path, "r", encoding="utf-8") as f:
            confirmed_genes = set(line.strip() for line in f if line.strip())
        ...
        if "all_bayes_factor" in gene_summary.columns:
            significant = set(
                gene_summary[gene_summary["all_bayes_factor"] > min_bf][
                    "gene_symbol"
                ].dropna()
            )
