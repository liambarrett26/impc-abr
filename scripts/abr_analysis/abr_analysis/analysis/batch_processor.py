...
class GeneBatchAnalyzer:
    ...
    def analyze_gene_group(self, group_info, sex_filter=None):
        ...
        try:
            ...
            print(
                f"Warning: Error analysing gene group {gene_symbol} ({allele_symbol}, {zygosity}, {center}){f', {sex_filter}' if sex_filter else ''}: {str(e)}"
            )
            return None
                ...
                for analysis_type, analysis in analyses.items():
                    if analysis:
                        ...
                        metrics = [
                            ...
                            "mean_mutant_logprob",
                            "std_mutant_logprob",
                            "mean_mutant_distance",
                            "mean_control_logprob",
                            "std_control_logprob",
    ...
    def create_gene_visualization(
        self, gene, output_dir, analysis_type="all", q_threshold=0.01
    ):
        ...
        for _, gene_row in gene_rows.iterrows():
            ...
            if (
                pd.isna(gene_row[f"{analysis_type}_q_value"])
                or gene_row[f"{analysis_type}_q_value"] >= q_threshold
            ):
                continue
            ...
            if (
                group_key not in self.mutant_profiles
                and f"{gene}_{allele}_{zygosity}_{center}_all"
                not in self.mutant_profiles
            ):
                continue
            ...
            try:
                ...
                plt.errorbar(
                    x,
                    control_data[self.freq_cols].mean(),
                ...
                plt.errorbar(
                    x,
                    mutant_data[self.freq_cols].mean(),
                ...
                if len(mutant_profiles) >= 3 and len(control_profiles) >= 20:
                    ...
                    mutant_log_probs = [
                        model.score(profile) for profile in mutant_profiles
                    ]
                    control_log_probs = [
                        model.score(profile) for profile in control_profiles
                    ]
                    ...
                    plt.hist(
                        mutant_log_probs, bins=20, alpha=0.5, label=gene, density=True
                    )
                ...
                plt.savefig(
                    gene_dir
                    / f"{gene}_{allele}_{zygosity}_{center}_{analysis_type}.png"
                )
                plt.close()
            except (
                ValueError,
                ...
                print(
                    f"Error creating visualisation for {gene} ({allele}, {zygosity}, {center}, {analysis_type}): {e}"
                )
    ...
    def create_visualizations(self, output_dir=".", q_threshold=0.01):
        ...
        # Use test_statistic
        x_vals = -np.log10(
            self.results["all_p_value"].replace([np.inf, -np.inf], np.nan)
        )
        ...
        allele_counts = (
            self.results[significant]
            .groupby(["allele_symbol", "gene_symbol"])
            .size()
            .reset_index()
        )
        allele_counts = (
            allele_counts.groupby("allele_symbol")
            .size()
        ...
        for analysis_type in ["all", "male", "female"]:
            sig_genes = self.results[
                self.results[f"{analysis_type}_q_value"] < q_threshold
            ]["gene_symbol"].unique()
        ...
        for gene in tqdm(
            ...
            for analysis_type in ["all", "male", "female"]:
                self.create_gene_visualization(
                    gene, output_dir, analysis_type, q_threshold
                )
    ...
    def compare_with_bowl(self, bowl_genes, q_threshold=0.01):
        ...
        for analysis_type in ["all", "male", "female"]:
            ...
            sig_genes = set(
                self.results[self.results[f"{analysis_type}_q_value"] < q_threshold][
                    "gene_symbol"
                ]
            )
    ...
    def compare_with_known_genes(
        self, confirmed_genes_path, candidate_genes_path, q_threshold=0.01
    ):
        """
