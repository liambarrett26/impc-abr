...
def run_batch_analysis(data_path, output_dir="results/multivariate"):
    ...
    comparisons = analyzer.compare_with_known_genes(
        confirmed_genes_path, candidate_genes_path
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for analysis_type in ["all", "male", "female"]:
            ...
            total_sig = (
                len(comp["found_in_confirmed"])
                + len(comp["found_in_candidate"])
                + len(comp["novel"])
            )
            f.write(f"Total significant genes: {total_sig}\n")
            f.write(
                f"Found in confirmed deafness genes: {len(comp['found_in_confirmed'])}\n"
            )
            f.write(
                f"Found in candidate deafness genes: {len(comp['found_in_candidate'])}\n"
            )
