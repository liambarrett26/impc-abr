        ...
        processed_data.append(
            {
                ...
                "equipment_manufacturer": metadata_dict.get(
                    "Equipment manufacturer", ""
                ),
                "equipment_model": metadata_dict.get("Equipment model", ""),
                "stimulus_level_range": metadata_dict.get(
                    "Range of stimulus levels used - Click", ""
                ),
...
def main():
    ...
    # Save under the repository ``data/raw`` directory; override with the
    # ``IMPC_ABR_OUTPUT`` environment variable to write elsewhere.
    project_root = Path(__file__).resolve().parents[2]
    default_path = project_root / "data" / "raw" / "impc_abr_data_with_waveforms.csv"
    save_path = os.environ.get("IMPC_ABR_OUTPUT", str(default_path))
